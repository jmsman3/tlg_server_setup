...
class HrPayslipWorkedDays(models.Model):
    ...
    # my custom logic for present days salary calculation _compute method overrite.
    def _compute_amount(self):
        for worked_days in self:
            payslip = worked_days.payslip_id

            # Skip if edited or not in allowed states
            if payslip.edited or payslip.state not in ['draft', 'verify']:
                continue

            # Skip if no contract, or OUT code, or credit time
            ...
            if payslip.wage_type == "hourly":
                # Hourly logic remains same
                worked_days.amount = contract.hourly_wage * worked_days.number_of_hours if worked_days.is_paid else 0
                print("worked_days_amount_houry",worked_days.amount)
            else:
                # NEW: Day-wise calculation based on monthly wage
                daily_wage = contract.contract_wage / 30
                worked_days.amount = daily_wage * worked_days.number_of_days if worked_days.is_paid else 0

                print("worked_days_amount",worked_days.amount)
        print("self amount",self)
        
    # original code from odoo
    # def _compute_amount(self):
    #     for worked_days in self:
